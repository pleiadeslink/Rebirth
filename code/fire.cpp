const sf::Color c_fire::colorPalette[37] {
...
c_fire::c_fire(float pixelSize, unsigned int width, unsigned int height, sf::Vector2f origin) {
    ...
    currentPixel = sf::RectangleShape(sf::Vector2f(m_pixelSize, m_pixelSize));;
}
...
void c_fire::setFireSourceStrength(int strength) {
    if (strength < 0)
        strength = 0;
    if (strength > 36)
        strength = 36;
...
unsigned int c_fire::getFireSourceStrength() const {
...
void c_fire::render(sf::RenderWindow& window) {
    for (unsigned int i=0; i < m_height; i++) {
        for (unsigned int j=0; j < m_width; j++) {
            if (!m_values[i][j] == 0) {
